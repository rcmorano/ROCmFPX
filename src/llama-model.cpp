...
void llama_model_base::load_hparams(llama_model_loader & ml) {
    ...
    for (uint32_t _il = 0; _il < hparams.n_layer; ++_il) {
        hparams.n_expert_used_impl = std::max(hparams.n_expert_used_impl, hparams.n_expert_used_arr[_il]);
    }
...
int32_t llama_model_n_nextn_heads(const llama_model * model) {
    ...
    return hp.nextn_predict_layers == 0 ? 0 : (int32_t)(hp.nextn_predict_layers / hp.nextn_predict_per_head);
}
